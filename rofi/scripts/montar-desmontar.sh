...
montar() {
	...
	if [[ -n "$aMontar" ]]; then
		...
		punto=$(udisksctl mount --no-user-interaction -b "$aMontar" 2>/dev/null | awk '{print $4}') && dunstify -t 4000 -i ~/.config/dunst/icons/apps/asa01.png "$aMontar montado en $punto" && exit 0
		dunstify -t 4000 -i ~/.config/dunst/icons/apps/angel02.png "Dispositivo montado!"
	else 
		dunstify -t 4000 -i ~/.config/dunst/icons/apps/asa00.png "Dispositivo no seleccionado!"
	fi
}
