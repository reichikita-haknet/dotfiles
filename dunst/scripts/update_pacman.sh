...
username=$(whoami)
updates=$(sleep 5;checkupdates | wc -l)
minutes=$(uptime | awk '{print $3}')
timeunit=$(uptime | awk '{print $4}' | cut -c '1-3')

if [ "$updates" -eq 0 ]; then
	dunstify "Update" "Todos los paquetes actualizados" -i ~/.config/dunst/icons/apps/makima00.png
else
	dunstify "Update" "Actualizaciones disponibles: $updates" -i ~/.config/dunst/icons/apps/makima01.png
fi
...
while true; do
	...
	if [ $nivel -le 20 ]; then
		notify-send -u critical -t 0 "Batería baja" "Conecta el cargador cuanto antes"
	fi
