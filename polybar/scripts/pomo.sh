#!/bin/bash
# Cada ciclo consta de se x numero de sesiones
# Al terminar cada ciclo inicia el descanso largo
# DEFAULT VALUES
...
timer() {
	...
	do
		...
		do
			...
			OUTPUT=$(timeout 1 notify-send "$2" "$BODY" -r $3  -a "$NAME" -i /home/reichikita/.config/dunst/icons/apps/pomodoro.png -A "Pause" -A "Stop" -A "Skip" & sleep 1)
			case $OUTPUT in
				0)	
					sh -c "$PAUSE_SCRIPT"
					while [ 1 ]; do
						OUT=$(timeout 1 notify-send "Paused!" "$BODY" -r $3 -u critical -a "$NAME" -i /home/reichikita/.config/dunst/icons/apps/pomodoro.png -A "Resume" & sleep 1)
						if [[ $OUT = 0 ]]; then
							break
						fi
					done
