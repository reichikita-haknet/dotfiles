	...
	"")
		...
		transparent = #00000000
			...
			echo "
                #!/bin/bash
		background="\"$bg\""
		# No puedes usar un "-" en el nombre de la variable
                backgroundAlt="\"$bgAlt\""
                transparent="\"#00000000\""
                foreground="\"$fg\""
                selected="\"$sel\""
                ...
                gray="\"$gy\""
                white="\"$w\""
			" > ~/.config/polybar/scripts/color-htb.sh
			# Para matar y recargar polybar y mas cositas que tendras que revisar 
			~/.config/polybar/scripts/launch.sh &
			~/.config/bspwm/scripts/pop_report -d 1000 -o "background-color: $bg" "border: 5px solid $sel" "font-family: 'Hack Nerd Font'" -m "POLYBAR RECARGADO"
		fi
